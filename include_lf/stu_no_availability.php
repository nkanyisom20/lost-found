<?php
  ...
  if(empty($student_no)){
    ...
    echo "<script>$('#submit_register').prop('disabled',true);</script>";
    }
  ...
  elseif(strlen($student_no) < 9){
    ...
    if($sql_select_num > 0)
      {
        echo "<span style='color:red'> Student No. already associated with another account .</span>";
        echo "<script>$('#submit_register').prop('disabled',true);</script>";
      }
    else
      {
      	echo "<span style='color:green'> Student No. Available for Registration .</span>";
        echo "<script>$('#submit_register').prop('disabled',false);</script>";
      }
    }
?>
